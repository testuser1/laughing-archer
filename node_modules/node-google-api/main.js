...
function makeMethod(_this, hostURL, method) {
	return function (options, cb) {
		// console.log(method + ' ' + resource + ' from ' + restAPI.name);
		var query = '?key='+ apiKey;
		// Insert the URL variables...
		console.log('0000', _this, method);
		var path = _this[method].vars.path;
		...
		sendRequest({
			...
			path: parsedUrl.path,
			port: parsedUrl.port,
			method: _this[method].vars.httpMethod
		}, function(data) {
		...
		getRest(apiList[api], function(restAPI) {
			...
			for(var resource in restAPI.resources) {
				...
				for(var method in restAPI.resources[resource].methods) {
					...
					var obj = apiObj[restAPI.name][resource];
					obj[method] = makeMethod(obj, hostURL, method);
					obj[method].vars = restAPI.resources[resource].methods[method];
				}
			}
...
function listAPIs(options, callback) {
	...
	var query = '?fields=items(discoveryRestUrl%2Cname%2Cversion%2Cdescription%2CdocumentationLink)';//&key='+apiKey;
	if(options) {
